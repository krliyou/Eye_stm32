...
#define micros() (rt_tick_get_millisecond() * 1500)
...
void drawlcd(rt_uint16_t x_start, rt_uint16_t y_start, \
...
void setup(void) {
  ...
  startTime = micros(); // For frame-rate calculation
}
...
//   digitalWrite(DISPLAY_DC, HIGH);                      // Data mode
  ...
  for(screenY=SCREEN_Y_START; screenY<SCREEN_Y_END; screenY++, scleraY++, irisY++) {
    ...
    drawlcd(0, (HW_SCREEN_HEIGHT - SCREEN_WIDTH) / 2+ screenY, 256, 1, buf);
    drawlcd(0, (HW_SCREEN_HEIGHT - SCREEN_WIDTH)/ 2 + screenY, 256, 1, buf);
...
//   }
  ...
  if(eyeInMotion) {                       // Currently moving?
    ...
    if(dt > eyeMoveDuration) {            // Time up?  Begin new move.
      ...
      do {                                // Pick new dest in circle
        eyeNewX = random(256, 1024);
        eyeNewY = random(256, 1024);
        dx      = (eyeNewX * 2) - 1023;
        dy      = (eyeNewY * 2) - 1023;
  ...
  if(eye[eyeIndex].blink.state) { // Eye currently blinking?
    ...
    if((eyeInfo[eyeIndex].wink >= 0) &&
       ((eyeInfo[eyeIndex].wink) == LOW)) { // Wink!
